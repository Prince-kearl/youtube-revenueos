  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Every significant admin action, with before/after context and outcome.
      </p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <FlatKpiCard
          title="Successful"
          value={String(entries.filter((e) => e.outcome === "Success").length)}
        />
        <FlatKpiCard
          title="Failed"
          value={String(entries.filter((e) => e.outcome === "Failed").length)}
        />
      </div>
            ...
            {sorted.map((e) => (
              <tr key={e.id} className="border-b border-border last:border-0 hover:bg-accent/30">
                ...
                <td className="px-3 py-3">
                  <span
                    className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium ${e.outcome === "Success" ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}
                  >
                    {e.outcome === "Success" ? (
