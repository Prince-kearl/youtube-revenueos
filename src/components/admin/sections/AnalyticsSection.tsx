...
const retention = [
  { month: "Month 1", pct: 100 },
  { month: "Month 2", pct: 68 },
  { month: "Month 3", pct: 54 },
  { month: "Month 4", pct: 47 },
  { month: "Month 5", pct: 42 },
  { month: "Month 6", pct: 39 },
];
const adoption = [
  { feature: "Link Tracking", pct: 91 },
  { feature: "AI Lab", pct: 78 },
  { feature: "Comment Automation", pct: 64 },
  { feature: "Brand Deals", pct: 52 },
  { feature: "Affiliate", pct: 34 },
  { feature: "Email Campaigns", pct: 21 },
];

export function AnalyticsSection() {
  const exportAs = (fmt: string) =>
    toast.success(`Exporting analytics as ${fmt}…`, {
      description: "Download isn't wired up in this preview.",
    });
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Churn, conversion, retention, and feature adoption across the platform.
          </p>
        </div>
        <div className="flex gap-1.5">
          {["CSV", "Excel", "PDF"].map((f) => (
            <button
              key={f}
              onClick={() => exportAs(f)}
              className="flex items-center gap-1.5 rounded-[var(--button-radius)] border border-border px-3 py-1.5 text-xs font-medium hover:bg-accent"
            >
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <FlatKpiCard title="Monthly Churn" value="3.8%" />
        <FlatKpiCard title="Visitor → Paid Conversion" value="6.1%" />
        <FlatKpiCard title="6-Month Retention" value="39%" />
        <FlatKpiCard title="Avg. Support Response" value="4.2m" />
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="relative rounded-xl card-gradient-outline p-5 lg:col-span-2">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Share of organizations that have used each feature in the last 30 days.
          </p>
