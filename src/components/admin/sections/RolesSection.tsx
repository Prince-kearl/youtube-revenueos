...
import { UserRolesManager, RoleSecurityNote } from "./UserRolesManager";
...
export function RolesSection() {
  ...
  // "User Roles" is the real, server-authenticated system (profiles.role) — the actual
  // authorization source. "Custom Roles" below is the pre-existing, separate concept of which
  // admin-console modules a staff member can see; kept as-is, untouched.
  const [tab, setTab] = useState<"user-roles" | "custom-roles">("user-roles");
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {tab === "user-roles"
              ? "Assign real roles to Tubify users — the actual authorization source."
              : "Create custom roles and control exactly which modules each one can reach."}
          </p>
        </div>
        {tab === "custom-roles" && (
          <button
      ...
      <div className="mt-4 flex items-center gap-1 rounded-full border border-border p-1 w-fit">
        <button
          onClick={() => setTab("user-roles")}
          className={`rounded-full px-4 py-1.5 text-xs font-medium ${tab === "user-roles" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          User Roles
        </button>
        <button
          onClick={() => setTab("custom-roles")}
          className={`rounded-full px-4 py-1.5 text-xs font-medium ${tab === "custom-roles" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          Custom Roles
        </button>
      </div>
          ...
          {roles.map((role) => (
            <div key={role.id} className="relative rounded-xl card-gradient-outline p-5">
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                {role.isSystem && (
                  <span className="flex items-center gap-1 rounded-md bg-accent px-2 py-1 text-[10px] font-medium text-muted-foreground">
              ...
              <div className="mt-3 flex flex-wrap gap-1">
                {role.permissions.slice(0, 4).map((p) => (
                  <span
                    key={p}
                    className="rounded-md bg-accent px-2 py-0.5 text-[10px] font-medium text-muted-foreground"
                  >
                    {p}
                  </span>
                ))}
              ...
              <div className="mt-4 flex gap-2 border-t border-border pt-3">
                <button
                  ...
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-[var(--button-radius)] border border-border py-1.5 text-xs font-medium hover:bg-accent"
                >
                ...
                {!role.isSystem && (
                  <button
                    ...
                    className="flex items-center justify-center rounded-[var(--button-radius)] border border-border px-2.5 py-1.5 text-xs font-medium text-destructive hover:bg-destructive/10"
                  >
      ...
      <ConfirmDialog
        ...
        description="Any admins assigned this role will lose the permissions it grants."
...
function RoleDialog({
  ...
  const [form, setForm] = useState<CustomRole>({
    id: "",
    name: "",
    description: "",
    isSystem: false,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Group permissions by module — assignees can only reach what's checked below.
          </DialogDescription>
        </DialogHeader>
