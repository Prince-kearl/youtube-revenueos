...
import { Plus, Send, Mail, Bell, Smartphone } from "lucide-react";
...
const channelIcon: Record<AnnouncementChannel, typeof Mail> = {
  "In-app": Bell,
  Email: Mail,
  Push: Smartphone,
};
...
export function CommunicationsSection() {
  ...
  const send = (a: Announcement) => {
    setItems((prev) =>
      prev.map((x) =>
        x.id === a.id ? { ...x, status: "Sent", date: new Date().toISOString().slice(0, 10) } : x,
      ),
    );
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Announcements, email campaigns, and in-app broadcasts.
          </p>
        </div>
        <button
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <FlatKpiCard title="Total Announcements" value={String(items.length)} />
        <FlatKpiCard title="Sent" value={String(items.filter((a) => a.status === "Sent").length)} />
        <FlatKpiCard
          title="Scheduled"
          value={String(items.filter((a) => a.status === "Scheduled").length)}
        />
        <FlatKpiCard
          title="Drafts"
          value={String(items.filter((a) => a.status === "Draft").length)}
        />
      </div>
              ...
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {a.audience} · {a.channel} · {a.date}
                  </p>
                </div>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <span
                  className={`rounded-md px-2.5 py-1 text-[11px] font-medium ${statusColor[a.status]}`}
                >
                  {a.status}
                </span>
                {a.status !== "Sent" && (
                  <button
                    onClick={() => send(a)}
                    className="flex items-center gap-1 rounded-[var(--button-radius)] border border-border px-2.5 py-1.5 text-xs font-medium hover:bg-accent"
                  >
      ...
      <CreateAnnouncementDialog
        ...
        onCreate={(a) => {
          ...
          log("Created announcement", "Communications", a.title);
        }}
      />
    </div>
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Saved as a draft — send it from the list once ready.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select value={audience} onValueChange={setAudience}>
                ...
                <SelectContent>
                  {["All Users", "Trial Users", "Starter Plan", "Pro Plan", "Scale Plan"].map(
                    (a) => (
