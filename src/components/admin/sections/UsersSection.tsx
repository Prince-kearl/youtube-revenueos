...
export function UsersSection() {
  ...
  const verify = (u: PlatformUser, field: "emailVerified" | "creatorVerified") => {
    ...
    toast.success(
      `${field === "emailVerified" ? "Email" : "Creator badge"} verified for ${u.name}`,
    );
  };
  const resetPassword = (u: PlatformUser) => {
    log("Reset password", "Users", u.name);
    toast.success(`Password reset email sent to ${u.email}`);
  };
  const forceLogout = (u: PlatformUser) => {
    log("Forced logout", "Users", u.name);
    toast.success(`All sessions for ${u.name} were revoked`);
  };
  const impersonate = (u: PlatformUser) => {
    log("Impersonated user", "Users", u.name);
    toast(`Impersonating ${u.name} — logged to the audit trail`, { icon: "🕵️" });
  };
  ...
  const bulkEmail = () => {
    log("Bulk email", "Users", `${selected.size} users`);
    toast.success(`Queued an email to ${selected.size} users`);
  };
  const bulkExport = () => {
    log("Bulk export", "Users", `${selected.size || users.length} users`);
    toast.success(`Exported ${selected.size || users.length} users as CSV`);
  };
    ...
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Complete user lifecycle management across every organization.
          </p>
        </div>
        <button
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <FlatKpiCard
          ...
          value={String(users.filter((u) => !u.emailVerified).length)}
        />
      </div>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                {PLATFORM_ROLES.map((r) => (
                  <DropdownMenuItem key={r} onSelect={() => bulkRole(r)}>
                    {r}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
          ...
          <tbody className="bg-card">
            {filtered.map((u) => (
              <tr key={u.id} className="border-b border-border last:border-0 hover:bg-accent/30">
                ...
                <td className="px-3 py-3.5 max-w-[180px] truncate text-muted-foreground">
                  {u.org}
                </td>
                ...
                <td className="px-3 py-3.5 text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      <DropdownMenuItem onSelect={() => impersonate(u)}>
                        <Eye className="mr-2 h-4 w-4" /> Impersonate user
                      </DropdownMenuItem>
                      <DropdownMenuItem onSelect={() => resetPassword(u)}>
                        <KeyRound className="mr-2 h-4 w-4" /> Reset password
                      </DropdownMenuItem>
                      <DropdownMenuItem onSelect={() => forceLogout(u)}>
                        <LogOut className="mr-2 h-4 w-4" /> Force logout
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      {!u.emailVerified && (
                        <DropdownMenuItem onSelect={() => verify(u, "emailVerified")}>
                          <BadgeCheck className="mr-2 h-4 w-4" /> Verify email
                        </DropdownMenuItem>
                      )}
                      {!u.creatorVerified && (
                        <DropdownMenuItem onSelect={() => verify(u, "creatorVerified")}>
                          <BadgeCheck className="mr-2 h-4 w-4" /> Verify creator
                        </DropdownMenuItem>
                      )}
                      <DropdownMenuSeparator />
                      ...
                      {u.status !== "Suspended" && (
                        <DropdownMenuItem onSelect={() => setStatus(u, "Suspended")}>
                          <PauseCircle className="mr-2 h-4 w-4" /> Suspend
                        </DropdownMenuItem>
                      )}
                      {u.status !== "Banned" && (
                        <DropdownMenuItem
                          onSelect={() => setStatus(u, "Banned")}
      ...
      <ConfirmDialog
        ...
        description="This permanently removes their accounts. This cannot be undone."
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new account directly — they'll show as Pending until they verify their email.
          </DialogDescription>
        </DialogHeader>
