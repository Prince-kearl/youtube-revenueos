  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Models, prompts, cost, and content moderation for every AI feature.
      </p>

      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <FlatKpiCard
          title="Enabled Models"
          value={String(models.filter((m) => m.enabled).length)}
        />
        <FlatKpiCard title="Blended Cost / 1K Tokens" value={`$${totalCost.toFixed(3)}`} />
        <FlatKpiCard title="Generations This Month" value="214.6K" />
        <FlatKpiCard title="Pending Moderation" value={String(pending)} />
      </div>
          ...
          <div key={m.id} className="relative rounded-xl card-gradient-outline p-5">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 rounded-lg bg-accent/30 p-3 text-center text-xs">
              <div>
                ...
                <p className="mt-0.5 font-semibold">{m.tokenLimit.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Rate Limit</p>
                <p className="mt-0.5 font-semibold">{m.rateLimitPerMin}/min</p>
              </div>
              <div>
                <p className="text-muted-foreground">Temperature</p>
                <p className="mt-0.5 font-semibold">{m.temperature}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Cost / 1K</p>
                <p className="mt-0.5 font-semibold">${m.costPer1kTokens.toFixed(3)}</p>
              </div>
            </div>
            {!m.isDefault && m.enabled && (
              <button
                onClick={() => setDefault(m)}
                className="mt-3 w-full rounded-[var(--button-radius)] border border-border py-1.5 text-xs font-medium hover:bg-accent"
              >
                Set as default
              </button>
            )}
        ...
        {prompts.map((p) => (
          ...
          >
            ...
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 rounded-[var(--button-radius)] border border-border px-2.5 py-1.5 text-xs font-medium hover:bg-accent"
            >
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`rounded-md px-2 py-0.5 text-[10px] font-medium ${modStatusColor[item.status]}`}
                >
                  {item.status}
                </span>
              </div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                {item.org} · {item.reason} · {item.date}
              </p>
            </div>
            {item.status === "Pending" && (
              <div className="flex shrink-0 gap-1.5">
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-[var(--button-radius)] border border-border px-2.5 py-1.5 text-xs font-medium text-destructive hover:bg-destructive/10"
                >
      ...
      <PromptDialog
        ...
        onOpenChange={(v) => !v && setEditingPrompt(null)}
        onSave={savePrompt}
      />
    </div>
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            onSave({ ...form, updated: new Date().toISOString().slice(0, 10) });
          }}
          className="space-y-3"
        >
