import { CheckCircle2, HardDrive, Play } from "lucide-react";
...
const services = [
  { name: "API", latency: "82ms" },
  { name: "Database", latency: "11ms" },
  { name: "Background Queue", latency: "3 jobs queued" },
  { name: "CDN", latency: "24ms" },
];
    ...
    setBackups((prev) => [
      {
        id: uid(),
        type: "Manual",
        size: `${(4 + Math.random()).toFixed(1)} GB`,
  ...
  return (
    <div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {services.map((s) => (
          <FlatKpiCard key={s.name} title={s.name} value="Operational" caption={s.latency} />
        ))}
      </div>
      ...
      <div className="relative mt-6 rounded-xl card-gradient-outline p-5">
        ...
        <div className="mt-3">
          <div className="flex justify-between text-xs text-muted-foreground">
            ...
            <span>{storageQuota} GB provisioned</span>
          </div>
      ...
      <div className="relative mt-4 rounded-xl card-gradient-outline">
        ...
        <div className="flex items-center justify-between p-5 pb-0">
          ...
          <button
            onClick={runBackup}
            className="flex items-center gap-1.5 rounded-[var(--button-radius)] bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90"
          >
