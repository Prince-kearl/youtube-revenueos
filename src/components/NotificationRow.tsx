...
// Fixed styling per type — matches the seeded examples this replaces (a deal alert is purple, a
// milestone is green, a payout is amber, a warning is red, an AI callout is purple, a reminder is
// blue), so nothing presentation-specific needs to be stored on the row itself.
const notifColor: Record<NotificationType, string> = {
  message: "bg-brand-purple/15 text-brand-purple",
  check: "bg-brand-green/15 text-brand-green",
  dollar: "bg-brand-amber/15 text-brand-amber",
  alert: "bg-brand-red/15 text-brand-red",
  zap: "bg-brand-purple/15 text-brand-purple",
  clock: "bg-brand-blue/15 text-brand-blue",
};
  ...
  return (
    <>
      ...
      >
        <button
          onClick={openDetail}
          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--button-radius)] ${notifColor[notification.type]}`}
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <div
              className={`flex h-10 w-10 items-center justify-center rounded-lg ${notifColor[notification.type]}`}
            >
