...
function ProfilePanel({ onOpenSecurity }: { onOpenSecurity: () => void }) {
  ...
  const [profileTab, setProfileTab] = useState<
    "Overview" | "Activity" | "Connections" | "Preferences"
  >("Overview");
    ...
    <div className="relative overflow-hidden rounded-xl card-gradient-outline shadow-sm">
      ...
      <div className="relative h-32 overflow-hidden bg-[#151515] sm:h-40">
        ...
        <button
          ...
          className="absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-full border border-white/20 bg-black/30 text-white hover:bg-black/50"
        >
      ...
      <div className="relative px-5 pb-0 sm:px-7">
          ...
          <div className="flex items-end gap-4">
            <div className="relative">
              {avatar ? (
                ...
                <div className="grid h-24 w-24 place-items-center rounded-full border-4 border-card bg-primary text-2xl font-bold text-primary-foreground shadow-lg sm:h-28 sm:w-28">
                  {name.charAt(0).toUpperCase()}
                </div>
              )}
              <button
                onClick={() => setEditor("avatar")}
        ...
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-muted-foreground">
          ...
          <span className="inline-flex items-center gap-1">
            <Crown className="h-3 w-3 text-primary" /> {role}
          </span>
        </div>

        <div className="mt-5 flex gap-5 overflow-x-auto border-b border-border">
          {["Overview", "Activity", "Connections", "Preferences"].map((tab) => (
            <button
              key={tab}
              onClick={() => setProfileTab(tab as typeof profileTab)}
              className={`relative shrink-0 pb-3 text-[11px] font-semibold transition-colors ${profileTab === tab ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              {tab}
              {profileTab === tab && (
          ...
          <>
            <div className="card-frost backdrop-blur-lg p-4">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                <div>
                  ...
                  <p className="mt-1 truncate text-xs font-semibold">{email || "Not available"}</p>
                </div>
                <div>
                  ...
                  {profileFieldsLoading ? (
                    ...
                    <p className="mt-1 text-xs font-semibold">{profile.location || "Not set"}</p>
                  )}
                </div>
                <div>
            ...
            <div className="card-frost backdrop-blur-lg p-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={onOpenSecurity} className="text-[10px] font-semibold text-primary">
                  Manage <ArrowRight className="ml-1 inline h-3 w-3" />
                </button>
              </div>
              <div className="mt-3 flex items-center justify-between rounded-lg bg-success/5 p-3">
                <span className="text-[10px] text-muted-foreground">Two-factor authentication</span>
                <button
              ...
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {channel
                      ? `${channel.subscriber_count.toLocaleString()} subscribers · ${(channel.video_count ?? 0).toLocaleString()} videos · ${(channel.view_count ?? 0).toLocaleString()} views`
                      : "Not connected"}
                  </p>
                </div>
              </div>
            ...
            <div className="card-frost backdrop-blur-lg p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[10px] text-muted-foreground">
                    Choose how Tubify looks on this device.
                  </p>
                </div>
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const body = (await response.json()) as { data?: ProfileData };
                    if (!response.ok || !body.data) throw new Error("save_failed");
...
function ProfileEditor({
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      if (typeof reader.result === "string")
        setValues((current) => ({ ...current, [imageField]: reader.result }));
    };
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error && error.message === "PROFILE_MIGRATION_REQUIRED"
          ? "Profile database migration is required"
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          <h3 id="profile-editor-title" className="text-lg font-semibold">
            {title}
          </h3>
          <button
            type="button"
          ...
          <div className="space-y-2">
            <label className="block text-xs font-semibold">
              ...
              <input
                ...
                onChange={(event) => setValues({ ...values, [imageField]: event.target.value })}
                className="mt-1 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm"
                placeholder="https://"
              />
            </label>
            <label className="inline-flex cursor-pointer items-center rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:border-primary">
              Upload image
              <input
                type="file"
                accept="image/*"
            ...
            <p className="text-[10px] text-muted-foreground">PNG, JPEG, WebP, or GIF up to 2 MB.</p>
          </div>
        )}
...
function AppearanceModeControl() {
  ...
  return (
    <div
      className="flex items-center gap-1 rounded-full border border-border bg-background p-1.5 shadow-inner"
      role="group"
  ...
  const accounts = [
    {
      ...
      description: loading
        ? "Checking connection…"
        : channels.length
          ? `${channels.length} linked channel${channels.length === 1 ? "" : "s"} · ${channels.find((item) => item.id === activeChannelId)?.channel_name ?? channels[0].channel_name}`
          : "Link one or more YouTube channels",
      connected: channels.length > 0,
      action: channels.length
        ? disconnectYoutube
        : () => {
            window.location.href = "/api/youtube/auth?returnTo=/settings";
          },
      actionLabel: channels.length
        ? disconnecting
          ? "Disconnecting…"
          : "Disconnect active"
        : "Connect",
    ...
    },
    {
      ...
      description: googleConnected
        ? "Connected through your YouTube Google authorization."
        : "Connect YouTube to authorize Analytics access.",
      connected: googleConnected,
      action: googleConnected
        ? () => navigate({ to: "/analytics" })
        : () => void connectExternal("google_analytics"),
    ...
    },
    {
      ...
      action: stripeConnected
        ? () => void disconnectExternal("stripe")
        : () => void connectExternal("stripe"),
      actionLabel:
    ...
    },
    {
      ...
      action: kitConnected
        ? () => void disconnectExternal("kit")
        : () => void connectExternal("kit"),
        ...
        <div className="mt-6 space-y-2">
          {accounts.map((account) =>
            account.rowLoading ? (
              <div
                key={account.title}
                className="flex flex-col gap-4 rounded-xl border border-border bg-background/70 p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4"
                aria-busy="true"
                aria-label={`Checking ${account.title} connection`}
              >
                <div className="flex min-w-0 items-center gap-3">
              ...
              <div
                key={account.title}
                className="group flex flex-col gap-4 rounded-xl border border-border bg-background/70 p-3 transition-colors hover:border-primary/40 sm:flex-row sm:items-center sm:justify-between sm:p-4"
              >
                <div className="flex min-w-0 items-center gap-3">
...
interface ConnectedYoutubeChannel {
  ...
  last_sync_status:
    "never_synced" | "syncing" | "success" | "partial" | "failed" | "reauth_required";
...
const YOUTUBE_CALLBACK_MESSAGES: Record<string, { type: "success" | "error"; text: string }> = {
  ...
  invalid_state: {
    type: "error",
    text: "That connection request expired or was invalid — try again.",
  },
...
function YouTubeIntegrationPanel() {
  ...
  const loadChannels = async () => {
    ...
    try {
      ...
      if (
        nextChannels.length &&
        (!activeChannelId || !nextChannels.some((channel) => channel.id === activeChannelId))
      ) {
  ...
  const updateIntegrationSetting = async (
    key: keyof typeof integrationSettings,
    value: boolean,
  ) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error && error.message === "YOUTUBE_NOT_CONNECTED"
          ...
          : "YouTube sync couldn't be completed.",
      );
    } finally {
      setSyncing(false);
    }
    ...
    <div className="relative rounded-xl card-gradient-outline p-6 space-y-6">
      ...
      {loading ? (
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Connect your YouTube channel to enable analytics and revenue sync.
                </p>
              </div>
            </div>
            <a
        ...
        channels.map((channel) => (
          <div
            key={channel.id}
            className={`rounded-xl border p-5 ${channel.id === activeChannelId ? "border-primary/60 bg-primary/5" : "border-border bg-accent/20"}`}
          >
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex min-w-0 items-center gap-4">
                {channel.thumbnail ? (
                  ...
                  <div className="grid h-14 w-14 shrink-0 place-items-center rounded-3xl bg-brand-red text-white">
                    <Youtube className="h-6 w-6" />
                  </div>
                )}
                <div className="min-w-0">
                  <p className="truncate font-semibold">{channel.channel_name}</p>
                  <p className="text-sm text-muted-foreground">
                    {channel.subscriber_count.toLocaleString()} subscribers
                  </p>
                  <p
                    className={`mt-1 text-sm ${channel.last_sync_status === "failed" || channel.last_sync_status === "reauth_required" ? "text-destructive" : channel.last_sync_status === "partial" ? "text-warning" : "text-success"}`}
                  >
                    {channel.last_sync_status === "syncing"
                      ? "Syncing…"
                      : channel.last_sync_status === "reauth_required"
                        ? "Reconnect required"
                        : channel.last_sync_status === "failed"
                          ? "Sync failed"
                          : channel.last_sync_status === "partial"
                            ? "Partially synced"
                            : channel.last_synced_at
                              ? `Connected · last synced ${new Date(channel.last_synced_at).toLocaleString()}`
              ...
              <div className="flex flex-wrap gap-2 lg:shrink-0">
                ...
                <button
                  ...
                  className="rounded-full border border-destructive/20 bg-destructive/10 px-4 py-2 text-sm font-semibold text-destructive hover:bg-destructive/15 disabled:opacity-60"
                >
      ...
      <div
        ...
        aria-label={settingsLoading ? "Loading YouTube sync settings" : undefined}
      >
        {settingsLoading ? (
          <>
            <ToggleRowSkeleton />
          ...
          (
            [
              [
                "auto_sync_videos",
                "Auto-sync videos",
                "Keep your channel videos up to date automatically.",
              ],
              [
                "import_analytics",
                "Import analytics data",
                "Fetch watch time, revenue, and engagement stats.",
              ],
              [
                "sync_comments",
                "Sync comment data",
                "Import comments for sentiment and reply tracking.",
              ],
              [
                "import_chapters",
                "Import chapter markers",
                "Pull chapter timestamps from your video descriptions.",
              ],
            ] as const
          ).map(([key, label, description]) => (
            <div
              key={key}
              className="flex flex-col gap-3 rounded-xl border border-border bg-accent/20 p-4 sm:flex-row sm:items-center sm:justify-between"
            >
              <div>
              ...
              <Switch
                ...
                disabled={loading || !channels.length || savingSetting !== null || syncing}
                onCheckedChange={(value) => void updateIntegrationSetting(key, value)}
              />
            </div>
          ))
        )}
  ...
  const notificationOptions = [
    {
      ...
      description: "Get notified when you hit new revenue records",
      enabled: true,
    },
    {
      key: "deals",
      label: "Brand deal updates",
      description: "Alerts for deal status changes and deadlines",
      enabled: true,
    },
    {
      key: "videos",
      label: "Video performance alerts",
      description: "CTR drops, viral spikes, and view milestones",
      enabled: true,
    },
    {
      key: "ai",
      label: "AI optimization ready",
      description: "When AI descriptions are ready for review",
      enabled: false,
    },
    {
      key: "digest",
      label: "Weekly digest",
      description: "Weekly performance summary via email",
      enabled: true,
    },
    {
      key: "payments",
      label: "Payment processed",
      description: "AdSense and affiliate payment confirmations",
      enabled: true,
    },
  ];
  const [preferences, setPreferences] = useLocalStore<Record<string, boolean>>(
    "yroos.notificationPreferences",
    Object.fromEntries(notificationOptions.map((item) => [item.key, item.enabled])),
  );
      ...
      <div className="mt-6 space-y-4">
        {notificationOptions.map((item) => (
          ...
          >
            ...
            <Switch
              checked={preferences[item.key] ?? item.enabled}
...
type BillingSubscriptionResponse = {
  data?: { subscription: BillingSubscription | null; plans: BillingPlan[] };
};
...
function BillingPanel() {
  ...
  useEffect(() => {
    ...
    fetch("/api/billing/subscription", { cache: "no-store" })
      .then(async (response) => {
        const body = (await response.json()) as BillingSubscriptionResponse;
  ...
  return (
    <div className="relative rounded-xl card-gradient-outline p-6 space-y-6">
      ...
      {status === "loading" ? (
        ...
        <p className="text-sm text-muted-foreground">We couldn't load your billing status.</p>
      ) : activePlan ? (
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
          ...
          <p className="text-3xl font-semibold">
            ...
            {subscription?.billing_interval === "year"
              ? ((activePlan.annualPriceCents ?? 0) / 1200).toFixed(2)
              : ((activePlan.monthlyPriceCents ?? 0) / 100).toFixed(0)}
      ...
      <div className="rounded-xl border border-border bg-accent/20 p-5 sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage your plan and payment method in the billing workspace.
          </p>
        </div>
        <Link
          to="/billing"
...
function SecurityPanel() {
  ...
  const loadMfaFactor = async () => {
    ...
    try {
      ...
      const factor = data.totp.find(
        (item: { id: string; status: string; friendly_name?: string | null }) =>
  ...
  const verifyMfaEnrollment = async () => {
    ...
    setMfaSubmitting(true);
    try {
      const { data: challenge, error: challengeError } = await challengeMfaFactor(
  ...
  const regenerateRecoveryCodes = async () => {
    if (
      !window.confirm(
        "Generating new recovery codes will permanently invalidate your existing recovery codes. Continue?",
      )
    )
      return;
    setMfaSubmitting(true);
        ...
        <div className="mt-5 card-frost backdrop-blur-lg p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                {mfaLoading && (
                  <Skeleton className="h-5 w-24 rounded-full" aria-label="Checking 2FA status" />
                )}
                {!mfaLoading && mfaFactor && (
                  <span className="rounded-full bg-success/10 px-2.5 py-1 text-[10px] font-bold text-success">
                    2FA is enabled <span aria-hidden="true">●</span>
                  </span>
                )}
              </div>
              <p className="mt-1 max-w-xl text-xs leading-5 text-muted-foreground">
                Add an extra layer of security to your account. You'll be asked for a verification
                code each time you sign in on a new device.
              </p>
            </div>
            {mfaLoading && (
              <Skeleton className="h-9 w-28 shrink-0 rounded-[var(--button-radius)]" />
            )}
            {!mfaLoading && !mfaFactor && (
              <button
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
                    Use an authenticator app like Google Authenticator, Microsoft Authenticator, or
                    1Password to generate 6-digit verification codes.
                  </p>
                  {mfaFactor && (
            ...
            <div className="rounded-xl border border-border bg-primary/5 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
                    Use recovery codes to sign in if you lose access to your authenticator app or
                    device.
                  </p>
                  {mfaFactor && (
                    <p className="mt-3 text-[10px] text-muted-foreground">
                      <strong className="text-foreground">
                        {recoveryRemaining ?? 0} codes available
                      </strong>
                    </p>
                  )}
                </div>
          ...
          {recoveryCodes && (
            <div className="mt-5 rounded-xl border border-warning/30 bg-warning/5 p-4">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                These codes are shown once. Store them somewhere secure in case you lose access to
              ...
              <div className="mt-3 flex gap-2">
                ...
                <button
                  onClick={() => {
                    const blob = new Blob(
                      [`Tubify recovery codes\n\n${recoveryCodes.join("\n")}\n`],
                      { type: "text/plain" },
                    );
                    const link = document.createElement("a");
                    link.href = URL.createObjectURL(blob);
                    link.download = "tubify-recovery-codes.txt";
                    link.click();
          ...
          {disableOpen && mfaFactor && (
            <div className="mt-5 rounded-xl border border-destructive/30 bg-destructive/5 p-4">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                This makes your account less secure. Enter a current authenticator code to continue.
              </p>
              <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                <input
                  ...
                  className="h-11 rounded-[var(--input-radius)] border border-border bg-background px-4 text-sm tracking-[0.3em] outline-none focus:border-primary"
                  placeholder="000000"
                />
                <button
                  onClick={() => void removeMfa()}
                  disabled={mfaSubmitting}
                  className="rounded-[var(--button-radius)] bg-destructive px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"
                >
                  Disable 2FA
                </button>
              </div>
          ...
          {[
            {
              ...
              detail: "Current session · Chrome on Windows · Lagos, Nigeria",
              color: "text-success bg-success/10",
            },
            {
              icon: ShieldCheck,
              title: "2FA enabled",
              detail: "Authenticator app · Security settings",
              color: "text-primary bg-primary/10",
            },
            {
              icon: KeyRound,
              title: "Recovery codes generated",
              detail: "10 codes generated · Security settings",
              color: "text-primary bg-primary/10",
            },
          ].map((event) => (
            <div key={event.title} className="flex items-center gap-3 py-3">
              <span
                className={`grid h-7 w-7 shrink-0 place-items-center rounded-full ${event.color}`}
              >
      ...
      <div className="relative flex flex-col gap-3 rounded-xl card-gradient-outline p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] text-muted-foreground">
              Lost access to your authenticator app or recovery codes?
            </p>
          </div>
        </div>
        <Link
        ...
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 sm:p-5 [&::-webkit-details-marker]:hidden">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              A simple extra check keeps your account protected.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Sign in to your Tubify account using your password.",
              },
              {
                n: "2",
              ...
              <div key={step.n} className="relative text-center">
                ...
                <p className="mx-auto mt-1 max-w-[170px] text-[10px] leading-4 text-muted-foreground">
                  {step.desc}
                </p>
                {step.n !== "3" && (
                  <ArrowRight className="absolute right-0 top-1 h-4 w-4 text-muted-foreground/50 max-md:hidden" />
                )}
              </div>
            ))}
          </div>
              ...
              {Array.from({ length: 6 }, (_, index) => (
                <input
                  ...
                  onChange={(event) => {
                    ...
                    if (digit && index < 5) verificationInputRefs.current[index + 1]?.focus();
                  }}
                  onKeyDown={(event) => {
  ...
  return (
    <div>
      ...
      <input
        readOnly={readOnly}
        value={value}
        className="h-11 w-full rounded-[var(--input-radius)] border border-border bg-accent/20 px-4 text-sm outline-none focus:border-primary read-only:cursor-default read-only:opacity-80"
      />
    </div>
  );
}
...
function DashboardBannerPanel() {
  ...
  const toggles: {
    key: "showName" | "showSubscribers" | "showAvatar" | "showVisitButton" | "showRecentPosts";
    label: string;
    description: string;
  }[] = [
    {
      key: "showName",
      label: "Channel name",
      description: "Show your YouTube channel name on the banner.",
    },
    {
      key: "showSubscribers",
      label: "Subscriber count",
      description: "Display your subscriber total next to the name.",
    },
    {
      key: "showAvatar",
      label: "Channel avatar",
      description: "Show your channel profile picture on the banner.",
    },
    {
      key: "showVisitButton",
      label: "Visit channel button",
      description: "Show a button that opens your YouTube channel.",
    },
    {
      key: "showRecentPosts",
      label: "Recent videos",
      description: "Show a preview grid of your recently published videos.",
    },
  ];
  ...
  const scopes = [
    {
      ...
      purpose: "Read your channel, videos, thumbnails, and public statistics.",
    },
    {
      name: "YouTube Analytics",
      scope: "yt-analytics.readonly",
      purpose: "Read views, watch time, engagement, and audience metrics.",
    },
    {
      name: "YouTube revenue analytics",
      scope: "yt-analytics-monetary.readonly",
      purpose: "Read estimated revenue and RPM where YouTube makes this information available.",
    },
  ];
  return (
    <div className="relative rounded-xl card-gradient-outline p-6 space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Tubify requests only the permissions needed to sync your connected YouTube channel and
          analytics.
        </p>
      </div>
      <div className="space-y-3">
        {scopes.map((s) => (
          <div
            key={s.name}
            className="flex flex-col gap-2 rounded-xl border border-border bg-accent/20 p-4 sm:flex-row sm:items-center sm:justify-between"
          >
            <div className="min-w-0">
      ...
      <div className="flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm">
        ...
        <p className="text-muted-foreground">
          Your YouTube connection can be removed at any time from Connected Accounts. Credentials
          are encrypted on the server and are never displayed here.
        </p>
      </div>
    </div>
  );
...
function CompliancePanel() {
  ...
  const deleteAccount = () => {
    clearAllStores();
    toast.success("Local workspace data cleared", {
      description: "Your locally stored Tubify data has been erased.",
    });
    setDeleteOpen(false);
    ...
    <div className="relative rounded-xl card-gradient-outline p-6 space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <button
          onClick={() => {
            const exportData = Object.fromEntries(
              Object.entries(localStorage).filter(([key]) => key.startsWith("yroos.")),
            );
        ...
        >
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Download your Tubify data as a JSON archive.
          </p>
        </button>
        <button
          onClick={() => {
            localStorage.removeItem("tubify_cookie_consent");
            toast.success("Cookie choice reset", {
              description: "The cookie notice will appear again on the public landing page.",
            });
          }}
          className="rounded-xl border border-border bg-accent/20 p-4 text-left hover:border-primary"
        >
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Control tracking, analytics, and AI-processing preferences.
          </p>
        </button>
        <button
        ...
        >
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Remove locally stored workspace data from this browser.
          </p>
        </button>
      </div>
      <div className="border-t border-border pt-5">
        ...
        <div className="mt-3 flex flex-col items-start gap-2 text-sm">
          ...
          <a href="/privacy#sharing" className="text-primary hover:underline">
