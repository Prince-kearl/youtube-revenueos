...
const defaults = { auto_sync_videos: true, import_analytics: true, sync_comments: false, import_chapters: true };
...
function parseChapters(description: string | null, durationSeconds: number | null) {
  ...
  const chapters: Array<{ title: string; startSeconds: number; endSeconds: number | null; position: number }> = [];
...
async function loadSettings(service: ReturnType<typeof createServiceSupabaseClient>, channelId: string): Promise<SyncSettings> {
  const { data } = await service.from("youtube_integration_settings").select(Object.keys(defaults).join(", ")).eq("channel_id", channelId).maybeSingle();
  return { ...defaults, ...(data ?? {}) } as SyncSettings;
}

async function syncChannel(service: ReturnType<typeof createServiceSupabaseClient>, channel: ChannelRow) {
  const claimed = await service.from("youtube_channels").update({ last_sync_status: "syncing", last_sync_error: null }).eq("id", channel.id).neq("last_sync_status", "syncing").select("id").maybeSingle();
  if (claimed.error || !claimed.data) return { status: "already_syncing" };

  const settings = await loadSettings(service, channel.id);
  const result = { videos: "skipped", analytics: "skipped", comments: "skipped", chapters: "skipped" } as Record<string, string>;
  ...
  try {
    ...
    await service.from("youtube_channels").update({
    ...
    if (settings.auto_sync_videos || settings.sync_comments || settings.import_chapters) {
      try {
        ...
        if (settings.auto_sync_videos) {
          const { error } = await service.from("videos").upsert(videos.map((video) => ({
    ...
    if (settings.import_analytics) {
      try {
        ...
        const { error } = await service.from("youtube_analytics_snapshots").upsert({ channel_id: channel.id, period_start: periodStart, period_end: periodEnd, payload }, { onConflict: "channel_id,period_start,period_end" });
    ...
    if (settings.sync_comments) {
      try {
        const comments = await fetchRecentYoutubeComments(accessToken, videos.map((video) => video.id));
        const { error } = await service.from("youtube_comments").upsert(comments.map((comment) => ({
          channel_id: channel.id,
    ...
    if (settings.import_chapters) {
      try {
        for (const video of videos) {
          ...
          await service.from("youtube_chapters").delete().eq("channel_id", channel.id).eq("youtube_video_id", video.id);
          if (chapters.length) {
            const { error } = await service.from("youtube_chapters").insert(chapters.map((chapter) => ({ channel_id: channel.id, youtube_video_id: video.id, ...chapter })));
    ...
    const status = message.includes("YOUTUBE_") && message.includes(":401") ? "reauth_required" : "failed";
    await service.from("youtube_channels").update({ last_sync_status: status, last_sync_error: status === "reauth_required" ? null : "sync_unavailable" }).eq("id", channel.id);
    return { status };
  }
}

async function getChannel(service: ReturnType<typeof createServiceSupabaseClient>, id: string) {
  const { data } = await service.from("youtube_channels").select("id, user_id, youtube_channel_id, access_token_ciphertext, refresh_token_ciphertext, token_expiry, last_sync_status").eq("id", id).single();
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const { data: owned } = await client.from("youtube_channels").select("id").order("connected_at", { ascending: false }).limit(1).maybeSingle();
      ...
      GET: async ({ request }) => {
        ...
        if (!cronSecret || request.headers.get("Authorization") !== `Bearer ${cronSecret}`) return json({ error: "AUTH_REQUIRED" }, { status: 401 });
        const service = createServiceSupabaseClient();
        const { data: channels } = await service.from("youtube_channels").select("id, user_id, youtube_channel_id, access_token_ciphertext, refresh_token_ciphertext, token_expiry, last_sync_status");
        const results = [];
        for (const channel of (channels ?? []) as ChannelRow[]) results.push({ channelId: channel.id, ...(await syncChannel(service, channel)) });
        return json({ results });
      },
    },
  },
