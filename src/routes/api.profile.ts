  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          const { data: profile, error } = existing
            ...
            : await client
                .from("profiles")
                .insert({ id: user.id, email: user.email ?? null, name, avatar })
                .select(
                  "id, email, name, avatar, role, location, website, bio, cover_url, banner_settings, created_at, updated_at",
                )
                .single();
      ...
      PATCH: async ({ request }) => {
        try {
          const { client, user } = await requireSessionUser(request);
          // Zod already strips any key profileSchema doesn't declare (there is no `role` field on
          // it), so `role` can't reach this update via the request body today — profiles_self_update
          // RLS (see 202609130002_rbac_data.sql) is the real backstop either way. `.strict()` makes
          // that explicit and turns any future accidental passthrough field into a 422 instead of a
          // silently-ignored value.
          const input = profileSchema.strict().parse(await request.json());
          ...
          if (error || !profile) {
            ...
            return json(
              { error: error?.code === "42703" ? "PROFILE_MIGRATION_REQUIRED" : "DATABASE_ERROR" },
              { status: 500 },
            );
          }
