      ...
      GET: async ({ request }) => {
        try {
          ...
          const { data, error } = await client
            ...
            .select(
              "id, provider, provider_account_id, account_name, metadata, connected_at, updated_at",
            )
      ...
      POST: async ({ request }) => {
        try {
          ...
          const response = new Response(
            JSON.stringify({ url: buildProviderAuthorizationUrl(provider, state) }),
            { headers: { "Content-Type": "application/json" } },
          );
          response.headers.append(
            "Set-Cookie",
            buildSetCookie("provider_oauth_state", `${provider}:${state}`, { maxAge: 600 }),
          );
...
export async function completeProviderConnection(request: Request, provider: ExternalProvider) {
  ...
  if (!state || expected !== `${provider}:${state}`)
    return redirect(`${returnTo}?integration=invalid_state`, request);
  const { user, workspaceId, setCookieHeaders } = await getWorkspaceContext(request);
